...
Close and restart actions repeatedly terminate every matching For Honor process and wait until the processes remain gone before reporting success or relaunching. This also clears leftover background instances that would otherwise make Ubisoft report that the game is already running.

The utility stays open for every action and only closes when the player closes its window. The title-bar settings button detects Steam and Ubisoft Connect installations and lets the player save a default launch platform; platform launches go through the selected client instead of an arbitrary duplicate executable. Once For Honor's actual game window is available, the in-game quick-actions overlay pops out automatically; it waits through the launcher/splash phase and appears only once per game session. The overlay selects the display containing the largest visible For Honor window, including borderless/fullscreen layouts on multi-monitor systems. It uses a borderless, draggable custom title strip with a fade-out close animation. `ForHonorQuickActions/Logo.ico` is compiled into the EXE and taskbar icon.
